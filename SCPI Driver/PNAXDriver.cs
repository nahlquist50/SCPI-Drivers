...
namespace SCPI {
    ...
    public class PNAX : SCPIDriver {
        ...
        public class NoiseFigureChannelClass : ChannelClass {
            
            // Variables
            private static List<string> _allowedMeasurementTypes = new List<string>(new string[] { "S11", "S21", "S12", "S22", "GammaOpt", "Rn", "NFMin", "NF", "ENR", "T-Eff" });
            private double _noiseBandwidth;
            private NoiseReceiverGainEnum _noiseReceiverGain;

            // Properties
            public AveragingClass Averaging
            { get; private set; }
            public NoiseAveragingClass NoiseAveraging
            { get; private set; }
            public FrequencyClass Frequency
            { get; private set; }
            public string[] AllowedMeasurementTypes
            {
                get { return _allowedMeasurementTypes.ToArray(); }
            }
            public double NoiseBandwidth
            {
                get { return this.GetNoiseBandwidth(); }
                set { this.SetNoiseBandwidth(value); }
            }
            public NoiseReceiverGainEnum NoiseReceiverGain
            {
                get { return this.GetNoiseReceiverGain(); }
                set { this.SetNoiseReceiverGain(value); }
            }

            // Constructor
            internal NoiseFigureChannelClass(PNAX ParentPNAX, int Number)
                : base(ParentPNAX, Number)
            {
            ...
            // Protected Methods
            protected virtual double GetNoiseBandwidth()
            {
                _parentPNAX.ClearEventRegisters();
                _parentPNAX.WriteString(String.Format("SENSe{0}:NOISe:BWIDth:RESolution?", Number));
                _parentPNAX.WaitForMeasurementToComplete(_parentPNAX.Timeout);
                _noiseBandwidth = (double)_parentPNAX.ReadNumber(IEEEASCIIType.ASCIIType_R8, true);
                return _noiseBandwidth;
            }
            protected virtual void SetNoiseBandwidth(double NoiseBandwidth)
            {
                _noiseBandwidth = NoiseBandwidth;
                _parentPNAX.WriteString(String.Format("SENSe{0}:NOISe:BWIDth:RESolution {1}", Number, _noiseBandwidth));
            }
            protected virtual NoiseReceiverGainEnum GetNoiseReceiverGain()
            {
                double retVal;
                _parentPNAX.ClearEventRegisters();
                _parentPNAX.WriteString(String.Format("SENSe{0}:NOISe:GAIN?", Number));
                _parentPNAX.WaitForMeasurementToComplete(_parentPNAX.Timeout);
                retVal = (double)_parentPNAX.ReadNumber(IEEEASCIIType.ASCIIType_R8, true);
                if (-1 < retVal && retVal < 1) {
                    _noiseReceiverGain = NoiseReceiverGainEnum.Low;
                } else if (14 < retVal && retVal < 16) {
                    _noiseReceiverGain = NoiseReceiverGainEnum.Medium;
                } else {
                    _noiseReceiverGain = NoiseReceiverGainEnum.High;
                }
                return _noiseReceiverGain;
            }
            protected virtual void SetNoiseReceiverGain(NoiseReceiverGainEnum NoiseReceiverGain)
            {
                _noiseReceiverGain = NoiseReceiverGain;
                switch (_noiseReceiverGain) {
                    case NoiseReceiverGainEnum.Low:
                        _parentPNAX.WriteString(String.Format("SENSe{0}:NOISe:GAIN 0", Number));
                        break;
                    case NoiseReceiverGainEnum.Medium:
                        _parentPNAX.WriteString(String.Format("SENSe{0}:NOISe:GAIN 15", Number));
                        break;
                    case NoiseReceiverGainEnum.High:
                        _parentPNAX.WriteString(String.Format("SENSe{0}:NOISe:GAIN 30", Number));
                        break;
                }
            }
            ...
            public override bool AddMeasurement(string MeasurementName, string MeasurementType, MeasurementFormatEnum MeasurementFormat = MeasurementFormatEnum.Logarithmic)
            {
                ...
                if (!_allowedMeasurementTypes.Contains(MeasurementType))
                    return false;

                _measurements.Add(MeasurementName, MeasurementType);
                _parentPNAX.WriteString(String.Format("CALCulate{0}:CUSTom:DEFine '{1}', 'NoiseFigure', '{2}'", Number, MeasurementName, MeasurementType));
                SetMeasurementFormat(MeasurementName, MeasurementFormat);
            ...
            public int Add(MeasurementClassEnum MeasurementClass)
            {
                ...
                switch (MeasurementClass) {
                    case MeasurementClassEnum.SParameters:
                        ...
                        break;
                    case MeasurementClassEnum.NoiseFigure:
                        _channels.Add(firstOpenChannelNumber, new NoiseFigureChannelClass(_parentPNAX, firstOpenChannelNumber));
                        break;
                }
        ...
        public enum ExternalTriggerSlopeEnum { Positive, Negative };
            // Measurement
        public enum MeasurementClassEnum { SParameters, NoiseFigure };
